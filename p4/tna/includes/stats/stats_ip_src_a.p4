control c_stats_ip_src_a(inout header_t hdr, inout ingress_metadata_a_t ig_md) {
    ...
    // Whisper: ts diff between curr and last packet.
    /* Register<bit<32>, _>(REG_SIZE_FLOW_KEY) reg_ip_src_ts_diff; */
    ...
    /*
    RegisterAction<_, _, bit<32>>(reg_ip_src_ts_diff) ract_ts_diff = {
        void apply(inout bit<32> value, out bit<32> result) {
            result = 0;
            if (value != 0) {
                value = ig_md.meta.current_ts - value;
            } else {
    ...
    */
    ...
    /*
    action ts_diff() {
        ig_md.stats_ip_src.ts_diff = ract_ts_diff.execute(ig_md.hash.ip_src);
    }
    */
    ...
    apply {
        ...
        // Whisper: update the ts diff between curr and last packet.
        // Uses hash value before the peregrine-kitnet modification (no decay factor for whisper).
        // ts_diff();
