control c_stats_ip_src(inout header_t hdr, inout ingress_metadata_t ig_md) {
    ...
    RegisterAction<_, _, bit<32>>(reg_ip_src_mean_sqr) ract_mean_sqr_calc = {
        void apply(inout bit<32> value, out bit<32> result) {
            value = square_mean.execute((bit<32>)hdr.peregrine.ip_src_mean);
            result = value;
        }
    };
	...
	action std_dev_calc() {
		hdr.peregrine.ip_src_std_dev = ract_std_dev_calc.execute(0)[15:0];
	}
    ...
    apply {
        ...
        mean.apply();
		hdr.peregrine.ip_src_mean = (ig_md.stats_ip_src.pkt_len)[15:0];
